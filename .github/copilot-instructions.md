...
- **Device Groups**: Full list via `/api/v1/device-groups/` (NOT `/api/v1/inventory/device-groups/`); modal uses group names for selection, backend resolves via `get_or_create`. Sync from Zabbix via `sync_device_groups_for_device()` auto-creates missing groups and sets ManyToMany relationship; if `monitoring_group` null, assigns first Zabbix group.
