...
Run: docker compose -f docker/docker-compose.yml exec web pytest \
    backend/tests/test_inventory_metrics.py -v
"""
...
class ViewSetMetricsInstrumentationTest(APITestCase):
    ...
    def test_site_list_metrics(self):
        ...
        for metric in REGISTRY.collect():
            if metric.name == "inventory_api_requests":
    ...
    def test_device_create_model_operation_metric(self):
        ...
        initial_count = 0
        for metric in REGISTRY.collect():
            if metric.name == "inventory_model_operations":
                for sample in metric.samples:
...
class CacheMetricsTest(APITestCase):
    ...
    def test_cache_hit_metric(self):
        ...
        initial_count = 0
        for metric in REGISTRY.collect():
            if metric.name == "inventory_cache_operations":
                for sample in metric.samples:
