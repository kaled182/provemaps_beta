"""
Management command to update uptime_item_key, cpu_usage_item_key and
memory_usage_item_key for devices by inspecting Zabbix items.
"""
...
class Command(BaseCommand):
    help = "Update uptime/cpu/memory item keys for devices from Zabbix"
    ...
    def handle(self, *args, **options):
        ...
        for device in devices:
            self.stdout.write(
                f"Device {device.id}: {device.name} (hostid={device.zabbix_hostid})"
            )

            if not device.zabbix_hostid:
                self.stdout.write(
                    self.style.WARNING("  ⚠ No Zabbix hostid, skipping")
                )
                skipped_count += 1
                continue
            ...
            try:
                ...
                if not raw_host_items:
                    self.stdout.write(
                        self.style.WARNING("  ⚠ No items found in Zabbix")
                    )
                    skipped_count += 1
                    continue
                ...
                for item in raw_host_items:
                    ...
                    if not cpu_key:
                        if any(
                            ...
                            pattern in key_lower
                            for pattern in ["duty", "load", "util", "usage"]
                        ):
                            cpu_key = key
                ...
                for item in raw_host_items:
                    ...
                    if not memory_key:
                        if any(
                            ...
                            pattern in key_lower
                            for pattern in ["percent", "util", "usage", "used"]
                        ):
                            memory_key = key
                ...
                if uptime_key or cpu_key or memory_key:
                    ...
                    old_mem = getattr(device, "memory_usage_item_key", "")

                    if dry_run:
                        self.stdout.write(self.style.SUCCESS("  ✓ Would update:"))
                        if uptime_key:
                            self.stdout.write(
                                f'    Uptime: "{old_uptime or "EMPTY"}" → "{uptime_key}"'
                            )
                        if cpu_key:
                            self.stdout.write(
                                f'    CPU: "{old_cpu or "EMPTY"}" → "{cpu_key}"'
                            )
                        if memory_key:
                            self.stdout.write(
                                f'    Memory: "{old_mem or "EMPTY"}" → "{memory_key}"'
                            )
                    else:
                        if uptime_key:
                        ...
                        if memory_key:
                            self.stdout.write(f"    Memory: {memory_key}")
                        updated_count += 1
                else:
                    self.stdout.write(
                        self.style.WARNING("  ⚠ No uptime/CPU/Memory keys detected")
                    )
