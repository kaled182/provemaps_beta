...
"""Celery tasks for the inventory routes domain."""
...
TASK_NAMESPACE = "inventory.routes"


@shared_task(bind=True, name=f"{TASK_NAMESPACE}.build_route")
def build_route(
    self: CeleryTask,
    route_id: int,
    force: bool = False,
...
@shared_task(bind=True, name=f"{TASK_NAMESPACE}.build_routes_batch")
...
@shared_task(bind=True, name=f"{TASK_NAMESPACE}.invalidate_route_cache")
...
@shared_task(bind=True, name=f"{TASK_NAMESPACE}.import_route_from_payload")
def import_route_from_payload(
    self: CeleryTask,
    payload: Dict[str, Any] | services.RouteImportPayload,
    created_by: str = "inventory.routes.task",
) -> dict[str, Any]:
...
@shared_task(bind=True, name=f"{TASK_NAMESPACE}.health_check")
def health_check(self: CeleryTask) -> dict[str, Any]:
    """Light-weight health check for the routes worker/queue."""

    logger.info("Task health_check executed")
...
def invalidate_route_cache_legacy(
    self: CeleryTask,
    route_id: int,
) -> dict[str, Any]:
...
def import_route_from_payload_legacy(
    self: CeleryTask,
    payload: Dict[str, Any] | services.RouteImportPayload,
...
__all__ = [
    ...
    "health_check",
    "build_route_legacy",
    "build_routes_batch_legacy",
    "invalidate_route_cache_legacy",
    "import_route_from_payload_legacy",
    "health_check_routes_builder_legacy",
]
