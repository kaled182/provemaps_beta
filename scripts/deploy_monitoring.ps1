...
Write-Host "`n📦 Building monitoring stack..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "`n❌ Failed to start monitoring stack" -ForegroundColor Red
    exit 1
}
...
Write-Host "`n🔍 Checking Prometheus health..." -ForegroundColor Yellow
$maxRetries = 10
$retryCount = 0
...
if (-not $prometheusReady) {
    Write-Host "⚠️  Prometheus health check timed out (may still be starting)" -ForegroundColor Yellow
}

# Wait for Grafana to be ready
Write-Host "`n🔍 Checking Grafana health..." -ForegroundColor Yellow
$retryCount = 0
$grafanaReady = $false
...
if (-not $grafanaReady) {
    Write-Host "⚠️  Grafana health check timed out (may still be starting)" -ForegroundColor Yellow
}
...
try {
    ...
    if ($activeTargets.Count -gt 0) {
        foreach ($target in $activeTargets) {
            Write-Host "   ✅ $($target.labels.job) - $($target.scrapeUrl)" -ForegroundColor Green
        }
    } else {
        Write-Host "   ⚠️  No active targets yet (may take 30s for first scrape)" -ForegroundColor Yellow
    }
} catch {
    Write-Host "   ⚠️  Could not fetch targets (Prometheus may still be initializing)" -ForegroundColor Yellow
}
...
Write-Host "🚨 Prometheus Alerts:" -ForegroundColor Cyan
...
Write-Host "✅ Monitoring stack is ready for Phase 1 (10% rollout) observation!`n" -ForegroundColor Green
